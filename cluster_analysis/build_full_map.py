...
<div id="wrap">
 <div id="side">
  ...
  <div class="sec">
   ...
   <div style="font-size:11px;color:#9fdca0;margin:2px 0 6px">✔️ كل تعديلاتك تُحفظ تلقائيًا في متصفحك وتبقى بعد إغلاق الصفحة.</div>
...
function saveState(){try{store[curKey]={CL,ptCl,hidden,newCount};
  ...
  localStorage.setItem(LSKEY,JSON.stringify(o));}catch(e){}}
function loadState(){try{const r=localStorage.getItem(LSKEY);if(!r)return false;
  const o=JSON.parse(r);
  for(const k in o.store){const s=o.store[k];
    store[k]={CL:s.CL,ptCl:s.ptCl,hidden:new Set(s.hidden||[]),newCount:s.newCount||0};}
...
function resetAll(){if(!confirm('استرجاع التقسيمة الأصلية وحذف كل تعديلاتك؟'))return;
  try{localStorage.removeItem(LSKEY);}catch(e){}
  for(const k in store)delete store[k];
  store['11-14']=buildLive('11-14');store['5-9']=buildLive('5-9');curKey='11-14';hideNone=false;
  const s=store[curKey];CL=s.CL;ptCl=s.ptCl;hidden=s.hidden;newCount=s.newCount;
...
if(!loadState()){store['11-14']=buildLive('11-14');
  const s=store['11-14'];CL=s.CL;ptCl=s.ptCl;hidden=s.hidden;newCount=s.newCount;curKey='11-14';}
{const v=document.getElementById('vnone');if(v)v.checked=!hideNone;}
updateDsUI();refreshSelectors();renderAll();
</script></body></html>"""
