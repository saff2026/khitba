...
function popupHtml(n){const p=byName[n],id=ptCl[n];const d=document.createElement('div');
  ...
  const ex=excluded.has(n);
  let s='<b>'+n+'</b><br>المنطقة: '+p.region+'<br>الفئة: '+p.cat;
  if(ex)s+='<br><b style="color:#ff8a8a">🚫 مستبعدة من التجميع</b>';
  else if(id&&CL[id]){const st=stats(CL[id].cities);const[v,vc]=verdict(st.mx/60,CL[id].cities.length);
    s+='<br>المجموعة: '+id+'<br>التقييم: <b style="color:'+vc+'">'+v+'</b>';}else s+='<br>بدون مجموعة';
  d.innerHTML=s;
  const mkBtn=(txt,bg,fn)=>{const b=document.createElement('button');b.textContent=txt;
    ...
    b.onclick=fn;d.appendChild(b);};
  if(ex){mkBtn('↩️ أرجعها للتجميع','#2e7d32',()=>includeCity(n));}
  else{
    if(id&&CL[id]&&CL[id].cities.length===1)mkBtn('🗑️ احذف هذه المجموعة','#7a2e2e',()=>ejectCity(n));
    if(!id||!CL[id])mkBtn('➕ اجعلها مجموعة مستقلة','#1b6ca8',()=>{const w=applyMove(n,'__new__');map.closePopup();document.getElementById('estat').innerHTML='➕ أنشئت <b>'+w+'</b>';});
    mkBtn('🚫 استبعدها من التجميع','#8a5a00',()=>excludeCity(n));
  }
  return d;}
